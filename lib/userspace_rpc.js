...
NotificationRouterFacet.prototype.register = function(methodPath, handler) {
  ...
  var topic = ':userId.$cloud.ninja.services.rpc.' + methodPath;
...
NotificationRouterFacet.prototype._handleIncomingRPC = function(handler, options, amqMsg) {
  ...
  var attemptRPC = when.try(function() {
    ...
    if (typeof message.method !== 'undefined' && message.method !== options.method) {
      return this._errors.methodNotAllowed(message.id, message.method, options.method);
    }
...
NotificationRouterFacet.prototype._errors = {
  ...
  methodNotAllowed: function(id, msgMethod, optsMethod) {
    return {"jsonrpc": "2.0", "error": {"code": 10000, "message": "The 'method' must not exist or must match the topic name."}, "id": id, msgMethod: msgMethod, optsMethod: optsMethod};
  },
};
